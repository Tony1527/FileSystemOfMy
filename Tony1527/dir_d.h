...
for(pdentry_lsibling = parse(parent_address),pdentry_child = parse(__childAddr);__childAddr!=EOF;pdentry_lsibling = pdentry_child,__childAddr=pdentry_child->d_rsibling,pdentry_child = __childAddr!=EOF?parse(__childAddr):NULL)	//这个宏的意思是将父节点的d_address读入，另外两个指针分别用来接收指向孩子的遍历指针pdentry_child，以及在初始化时指向父节点pdentry_lsibling，后面这个指针用来获得当前正在读入节点的d_address



critical_dir* normal_cd;
...
d_address _applyDentry_d();				//3.申请一项目录项
...
Pdentry _rmfile_d(char* filepath);			//7.删除某一目录项,主要调用了上面的_recurse_rm_d()
...
void InitCDBlocks(critical_dir* cdir){
	...
	memset(cdir->padding,0,8*sizeof(char));
}
...
Pdentry inline parseDArress(d_address dAddr){
	...
	if(dAddr == FILEPATH_ERROR || dAddr == FILEPATH_INVALID)
		return NULL;
	for(i=0;i<cd_number;i++){/*此处在吴大师写好之后需要修改*/
...
Pdentry _mkfile_d(char* filepath){	
	...
	d_address new_dir = _applyDentry_d();
